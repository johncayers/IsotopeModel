"""
Kernel density estimate (KDE) plot of d29Si for zircon analyses grouped by
"Replaced" category (Y, P, N), overlaid in a single chart.

Each analysis is represented as a Gaussian kernel centered on its d29Si
...
"Y" analyses carry uncertainties ~1000x larger than "N", so without this
rescaling the Y and P curves are invisible next to N. Curve height is
therefore comparable in shape only, not in sample size or precision.

d29Si spans several orders of magnitude across categories (roughly -60 to
+400 for N/P, but +1,200 to +36,800 for Y), so the x-axis uses a symmetric
log ("symlog") scale: linear near zero, logarithmic beyond, on both the
...
OUT_PNG = "KDE_YPN_d29Si.png"
OUT_PDF = "KDE_YPN_d29Si.pdf"
...
# ---- Build the (symlog-friendly) value axis --------------------------------
...
# Each category's uncertainties span orders of magnitude (Y's are ~1000x N's),
# so true relative-density curves would make Y and P invisible next to N.
# Normalize each curve to its own peak = 1 so all three are comparably visible;
# curve height no longer reflects sample size or analytical precision.
